...
const ComboBox = (props) => {
    ...
    const filteredOptions =
      query === ''
        ...
        : props.options.filter((option) => {
            return option[props.displayField].toLowerCase().includes(query.toLowerCase())
          })

    return (
    <Combobox as="div" value={props.selected} onChange={props.onChange} multiple={props.multiple}>
    ...
    <div className="relative mt-1">
      <Combobox.Input
        ...
        displayValue={(optIds) => optIds.map((optId) => props.options.find((availOpt) => availOpt.id === optId)[props.displayField]).join(", ")}
      />
